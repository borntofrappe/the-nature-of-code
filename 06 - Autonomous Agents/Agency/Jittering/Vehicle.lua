...
function Vehicle:jitter(target)
  local desiredVelocity = LVector:subtract(target.position, self.position)
  local angle = target.satellite.angle
  local x = math.cos(angle) * (target.r + target.satellite.distance)
  local y = math.sin(angle) * (target.r + target.satellite.distance)
  desiredVelocity:add(LVector:new(x, y))
