...
function Tree:init()
  ...
  local offset = math.random(NOISE_INITIAL_MAX)
  local offsetIncrement = NOISE_INCREMENT
...
function Tree:update(dt)
  for i, line in ipairs(self.generations[#self.generations].lines) do
    local noiseX = (love.math.noise(self.offset) - 0.5) * dt * UPDATE_SPEED
    local noiseY = (love.math.noise(self.offset + NOISE_INITIAL_MAX) - 0.5) * dt * UPDATE_SPEED
    local noise = LVector:new(noiseX, noiseY)
    line.finish:add(noise)
